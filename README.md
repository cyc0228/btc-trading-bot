...
│                                                                          │
│   ┌─────────────────────┐        ┌──────────────────────┐   ┌────────────────┐  │
│   │   btc_monitor.py      │──調用─▶│   risk_manager.py      │   │     api.py       │  │
│   │  (Docker 容器)         │        │  (由 btc_monitor.py 引入) │   │  (Docker 容器)    │  │
│   │                       │        │                        │   │                  │  │
│   │  - 抓取K線/計算MA20      │        │  - 指數退避重試機制        │   │  - GET /          │  │
│   │  - 策略判斷/下單         │        │  - 2% 硬性止損判斷        │   │  - GET /trades     │  │
│   │  - 持倉狀態還原          │        └──────────────────────┘   │  - GET /status      │  │
│   └──────────┬──────────┘                                     │  - GET /balance     │  │
│              │                                                 └────────┬─────────┘  │
└──────────────┼──────────────────────────────────────────────────────────┼─────────────┘
...
機器人主程式與查詢 API 是**兩個獨立運行的 Docker 容器**，各自用 `docker-compose` 定義並同時啟動，部署於 AWS EC2 (Ubuntu) 上 24 小時運行。兩者透過共用的 PostgreSQL 資料庫溝通，各自獨立連線，不直接互相呼叫。`risk_manager.py` 並非獨立容器，而是被 `btc_monitor.py` 引入使用的風控模組。
...
- 提供查詢介面供 `btc_monitor.py`、`api.py` 共用
...
- 尚未串接批次績效分析與排程機制（規劃導入 Airflow，定期彙整交易紀錄、產出績效報表）
- 下單重試機制目前對所有 API 呼叫一視同仁，尚未區分冪等（查詢）與非冪等（下單）操作，實盤環境需另外處理重複下單風險
- 尚未有資料一致性核對機制，未來規劃定期比對資料庫紀錄與交易所真實餘額，主動偵測落差
- 監控告警目前僅涵蓋交易成功通知，未來可擴充至異常事件（如連續重試失敗、資料落差）的主動告警
